...
describe("calculatePrice", () => {
  it("charges the R32 base price for a distance of 1km or less, for every non-removal service", () => {
    for (const serviceType of ["parcel_delivery", "laundry", "legal_logistics"] as const) {
      expect(calculatePrice(serviceType, 1)).toBe(32);
      expect(calculatePrice(serviceType, 0.5)).toBe(32);
    }
  });

  it("adds R10/km beyond the first km, for every non-removal service", () => {
    for (const serviceType of ["parcel_delivery", "laundry", "legal_logistics"] as const) {
      expect(calculatePrice(serviceType, 5)).toBe(32 + 4 * 10);
    }
  });

  it("defaults to the legal_logistics base price when no service type is given", () => {
    expect(calculatePrice(null, 1)).toBe(32);
    expect(calculatePrice(undefined, 5)).toBe(32 + 4 * 10);
  });

  it("uses the R32 base for motorcycle/hatchback/sedan removal vehicles, same as every other service", () => {
    expect(calculatePrice("home_office_removal", 1, "motorcycle")).toBe(32);
    expect(calculatePrice("home_office_removal", 1, "hatchback")).toBe(32);
    expect(calculatePrice("home_office_removal", 1, "sedan")).toBe(32);
    expect(calculatePrice("home_office_removal", 5, "sedan")).toBe(32 + 4 * 10);
  });
  ...
  it("ignores vehicleType for services other than home_office_removal", () => {
    expect(calculatePrice("parcel_delivery", 5, "half_ton_bakkie")).toBe(32 + 4 * 10);
  });

  it("accepts distance as a string, matching how it's read off the booking form", () => {
    expect(calculatePrice("parcel_delivery", "5")).toBe(32 + 4 * 10);
  });
});
